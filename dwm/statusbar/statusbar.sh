...
refresh() {
  ...
  xsetroot -name "$_wifi$_cpu$_mem$_date$_bat" # 更新状态栏
}
...
cron() {
  ...
  while true; do
    ...
    [ $((i % 20)) -eq 0 ] && to=(${to[@]} cpu mem) # 每 20秒  更新 cpu mem vol icons
    [ $((i % 300)) -eq 0 ] && to=(${to[@]} bat)              # 每 300秒 更新 bat
    [ $((i % 5)) -eq 0 ] && to=(${to[@]} date)         # 每 5秒   更新 date
    [ $i -lt 30 ] && to=(wifi cpu mem date bat)    # 前 30秒  更新所有模块
...
update)
  ...
  update $*
  ;;
updateall | check) update wifi cpu mem date bat ;;
