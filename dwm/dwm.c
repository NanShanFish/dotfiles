...
static void my_grid(Monitor *m, uint gappo, uint gappi);
...
void focus(Client *c) {
  ...
  if (c) {
    ...
    XSetWindowBorder(
        dpy, c->win,
        scheme[c->isglobal ? SchemeSelGlobal : SchemeSel][ColBorder].pixel);
    setfocus(c);
  } else {
...
void manage(Window w, XWindowAttributes *wa) {
  ...
  updatetitle(c);
  // managetransientwin 管理transient window，此时transient
  // window也会调用applyrules
  if (!managetransientwin && XGetTransientForHint(dpy, w, &trans) &&
      (t = wintoclient(trans))) {
    c->mon = t->mon;
    c->tags = t->tags;
  } else {
...
void magicgrid(Monitor *m) { my_grid(m, gappo, gappi); }

void overview(Monitor *m) { grid(m, overviewgappo, overviewgappi); }

void my_grid(Monitor *m, uint gappo, uint gappi) {
  unsigned int i, n;
  ...
  for (rows = 0; rows <= n / 2; rows++)
    if (rows * rows >= n) // 使col^2刚刚 > windows_num 2x2 3x3
      break;
  cols = (rows && (rows - 1) * rows >= n) ? rows - 1 : rows;
  cw = (m->ww - 2 * gappo - (rows - 1) * gappi) / rows;

  overcols = n % cols;
  c = nexttiled(m->clients);
  // 第一列
  if (overcols) {
    ch = (m->wh - 2 * gappo - (overcols - 1) * gappi) / overcols;
    for (i = 0; i < overcols; c = nexttiled(c->next), i++) {
      cy = m->wy + i * (ch + gappi);
      resize(c, m->wx + gappo, cy + gappo, cw - 2 * c->bw, ch - 2 * c->bw, 0);
    }
  }
  ch = (m->wh - 2 * gappo - (cols - 1) * gappi) / cols;
  for (i = (overcols ? 1 : 0) * cols; c; c = nexttiled(c->next), i++) {
...
void grid(Monitor *m, uint gappo, uint gappi) {
  ...
  overcols = n % cols;
  // if (overcols) dy = (m->ww - overcols * cw - (overcols - 1) * gappi) / 2 -
  // gappo;
  if (overcols)
    dy = (m->wh - overcols * ch - (overcols - 1) * gappi) / 2 - gappo;
